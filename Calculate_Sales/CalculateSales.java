...
public class CalculateSales {
	public static void main(String[] args){
		...
		//1.支店定義ファイル読込（メソッド）
		if(!printIn(args[0], "branch.lst", branchNameMap, branchSaleMap, "支店定義", "^\\d{3}$")){
			return;
		}
		//2.商品定義ファイル読込（メソッド）
		if(!printIn(args[0], "commodity.lst", commodityNameMap, commoditySaleMap, "商品定義", "^\\w{8}$")){
			return;
		}
	...
	//引数にディレクトリ、ファイル名、Mapの名前、Mapの金額、ファイル定義名、正規表現での桁数
		public static boolean printIn(String dirPath, String fileName, HashMap<String, String>names, HashMap<String, Long>sales, String definefile, String coad){
			...
			if(!dirin.exists()){ //ファイルの存在確認
				System.out.println(definefile + "ファイルが存在しません");
				return false;
			}
			...
			try{
				...
				while ((dirinS = br.readLine()) != null){ //ファイルを1行ずつ読込
					...
					if((s.length == 2) && (s[0].matches(coad)) && (!(0 == s[1].length()))){
						...
						System.out.println(definefile + "ファイルのフォーマットが不正です");
						return false;
					}
				}
			} catch(IOException e){
